...
// End init_form_fields()
    ...
    public function create_reference_transaction($token, $order) {
        ...
        $is_prices_include_tax = $order->get_prices_include_tax();
        if (($is_prices_include_tax || $order->get_total_discount() > 0 || $length_error > 1) && $order->get_subtotal() > 0) {
            ...
            if (sizeof($order->get_items()) > 0) {
                ...
                if ($order->get_subtotal() == 0) {
                    $paypal_args['L_COST0'] = number_format($order->get_shipping_total() + $order->get_shipping_tax(), 2, '.', '');
                } else {
                    $paypal_args['FREIGHTAMT'] = number_format($order->get_shipping_total() + $order->get_shipping_tax(), 2, '.', '');
                    $paypal_args['L_COST0'] = number_format($order->get_total() - round($order->get_shipping_total() + $order->get_shipping_tax(), 2), 2, '.', '');
                }
            ...
            if (sizeof($order->get_items()) > 0 && $order->get_subtotal() > 0) {
                ...
                $paypal_args['L_QTY0'] = 1;
                $paypal_args['L_COST0'] = number_format($order->get_shipping_total() + $order->get_shipping_tax(), 2, '.', '');
                $paypal_args['ITEMAMT'] = number_format($order->get_shipping_total() + $order->get_shipping_tax(), 2, '.', '');
            }
        }
        try {
            $postData = '';
