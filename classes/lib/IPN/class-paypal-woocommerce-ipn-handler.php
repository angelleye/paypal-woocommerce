...
class PayPal_WooCoomerce_IPN_Handler {
    ...
    public function angelleye_paypal_woocommerce_order_status_handler($posted) {
        if (!empty($posted['custom']) && ( $order = $this->get_paypal_order($posted['custom']) )) {
            ...
            $this->add_log->add( 'paypal_woocommerce_ipn', 'Found order #' . $order->id);
    ...
    protected function validate_currency($order, $currency) {
        if ($order->get_order_currency() != $currency) {
            $this->add_log->add( 'paypal_woocommerce_ipn', 'Payment error: Currencies do not match (sent "' . $order->get_order_currency() . '" | returned "' . $currency . '")');
    ...
    protected function payment_status_completed($order, $posted) {
        if ($order->has_status('completed')) {
            $this->add_log->add( 'paypal_woocommerce_ipn', 'Aborting, Order #' . $order->id . ' is already complete.');
            exit;
        }
    ...
    protected function payment_status_refunded($order, $posted) {
        if ($order->get_total() == ( $posted['mc_gross'] * -1 )) {
            ...
            $this->send_ipn_email_notification(
                    sprintf(__('Payment for order %s refunded', 'woo-paypal-plus'), '<a class="link" href="' . esc_url(admin_url('post.php?post=' . $order->id . '&action=edit')) . '">' . $order->get_order_number() . '</a>'), sprintf(__('Order #%s has been marked as refunded - PayPal reason code: %s', 'woo-paypal-plus'), $order->get_order_number(), $posted['reason_code'])
            );
        }
    }
    ...
    protected function payment_status_reversed($order, $posted) {
        ...
        $this->send_ipn_email_notification(
                sprintf(__('Payment for order %s reversed', 'woo-paypal-plus'), '<a class="link" href="' . esc_url(admin_url('post.php?post=' . $order->id . '&action=edit')) . '">' . $order->get_order_number() . '</a>'), sprintf(__('Order #%s has been marked on-hold due to a reversal - PayPal reason code: %s', 'woo-paypal-plus'), $order->get_order_number(), wc_clean($posted['reason_code']))
        );
    }
    ...
    protected function payment_status_canceled_reversal($order, $posted) {
        $this->send_ipn_email_notification(
                sprintf(__('Reversal cancelled for order #%s', 'woo-paypal-plus'), $order->get_order_number()), sprintf(__('Order #%s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %s', 'woo-paypal-plus'), $order->get_order_number(), esc_url(admin_url('post.php?post=' . $order->id . '&action=edit')))
        );
    }
