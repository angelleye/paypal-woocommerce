...
class WC_Gateway_PayPal_Plus_AngellEYE extends WC_Payment_Gateway {
    ...
    function get_approvalurl($order_id) {
        ...
        include_once( 'lib/PayPalPlus.php'); //our PayPal functions
        ...
        try { // try a payment request
            ...
            $_SESSION["get_order_details"] = $order_details;

            $result = create_paypal_payment(number_format( $order->get_total(), 2, '.', '' ), PP_CURRENCY, '', $order_details['OrderItems'], $redirect_url, CANCEL_URL);
...
// End init_form_fields()
    ...
    public function executepay() {
        ...
        if (! empty( $_GET['pp_action'] ) && $_GET['pp_action'] == 'executepay') {
            ...
            if (empty($_SESSION['payment_args']['token']) || empty($_SESSION['payment_args']['PayerID']) || empty($_SESSION['payment_args']['paymentId'])) return;
