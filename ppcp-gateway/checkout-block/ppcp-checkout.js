    ...
    function (e, t, n) {
        ...
        const render = () => {
            ...
            return shouldShowDiv && (
                createElement(ExperimentalOrderMeta, null,
                    createElement("div", {className: "angelleye_ppcp_message_cart"})
                )
            );
        };

        if (jQuery.inArray("cart", ppcp_settings.pay_later_messaging_page_type) !== -1) {
            registerPlugin('wc-ppcp', {render, scope: 'woocommerce-cart'});
        } else if (jQuery.inArray("payment", ppcp_settings.pay_later_messaging_page_type) !== -1) {
            registerPlugin('wc-ppcp', {render, scope: 'woocommerce-checkout'});
        }
    }
]);
