...
class Cartflows_Pro_Gateway_PayPal_PPCP_CC_AngellEYE extends Cartflows_Pro_Paypal_Gateway_helper {
    ...
    public function get_purchase_units($order, $offer_product, $args) {
        ...
        $purchase_unit = array(
            ...
            'amount' => array(
                'currency_code' => add_filter('angelleye_ppcp_woocommerce_currency', $args['order_currency'], $offer_product['price']),
    ...
    public function get_item_breakdown($order, $offer_product) {
        ...
        $breakdown['item_total'] = array(
            'currency_code' => add_filter('angelleye_ppcp_woocommerce_currency', wcf_pro()->wc_common->get_currency($order), $offer_product['unit_price_tax']),
            'value' => $offer_product['unit_price_tax'],
        );
        if (!empty($offer_product['shipping_fee'])) {
            $breakdown['shipping'] = array(
                'currency_code' => add_filter('angelleye_ppcp_woocommerce_currency', wcf_pro()->wc_common->get_currency($order), $offer_product['shipping_fee_tax']),
