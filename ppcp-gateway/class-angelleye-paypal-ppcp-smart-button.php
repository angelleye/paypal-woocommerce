...
class AngellEYE_PayPal_PPCP_Smart_Button {
    ...
    public function angelleye_ppcp_all_web_request($response, $response_text, $Requests_text, $parsed_args, $url) {
        $this->api_log->log($url, 'error');
        $this->api_log->log($Requests_text, 'error');
        $this->api_log->log($parsed_args, 'error');
        $this->api_log->log($response_text, 'error');
        $this->api_log->log($response, 'error');
    ...
    public function display_paypal_button_checkout_page() {
        if (angelleye_ppcp_has_active_session() === false) {
            ...
            if (is_checkout_pay_page() === false) {
                $separator = '<div class="angelleye_ppcp-proceed-to-checkout-button-separator checkout_cc_separator">&mdash;&mdash; ' . __('OR', 'paypal-for-woocommerce') . ' &mdash;&mdash;</div>';
            }
