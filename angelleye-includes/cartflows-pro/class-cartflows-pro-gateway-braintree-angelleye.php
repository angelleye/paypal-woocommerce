...
class Cartflows_Pro_Gateway_Braintree_AngellEYE {
    ...
    public function __construct() {
        add_filter('angelleye_braintree_store_in_vault_on_success', [$this, 'tokenize_if_required'], 10, 1);
    }
    ...
    public function process_offer_payment($order, $product) {
        try {
            ...
            if (!empty($gateway->merchant_account_id)) {
    ...
    public function process_offer_refund($order, $offer_data) {
        ...
        if (isset($transaction->status) && $transaction->status == 'submitted_for_settlement') {
            try {
                ...
                if ($result->success) {
                    do_action('angelleye_paypal_response_data', $result, $request_data = [], '1', $gateway->sandbox, false, 'braintree');
                    
                    $braintree_refunded_id = [];
