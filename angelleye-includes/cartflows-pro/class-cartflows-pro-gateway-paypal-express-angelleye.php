...
class Cartflows_Pro_Gateway_Paypal_Express_Angelleye extends Cartflows_Pro_Paypal_Gateway_helper {
    ...
    public function __construct() {
        ...
        add_filter('angelleye_woocommerce_express_checkout_set_express_checkout_request_args', [$this, 'modify_paypal_arguments'], 999);
        add_filter('angelleye_woocommerce_express_checkout_do_reference_transaction_request_args', [$this, 'modify_do_reference_transaction_request_paypal_arguments'], 999);

        add_action('cartflows_offer_subscription_created', [$this, 'add_subscription_payment_meta_for_paypal_express'], 10, 3);
    }
    ...
    public function payment_scripts() {
        if (_is_wcf_base_offer_type() && !wcf_pro()->utils->is_reference_transaction() && $this->has_paypal_gateway()) {
            wp_enqueue_script(
                'wcf-paypal-script', 'https://www.paypalobjects.com/api/checkout.js', ['jquery'], CARTFLOWS_PRO_VER, true
            );
            ...
            var $wcf_angelleye = {
                init: function () {
                    ...
                    window.paypalCheckoutReady = function () {
                        paypal.checkout.setup('<?php echo $this->get_woo_payer_id(); ?>', {
    ...
    public function modify_paypal_arguments($data) {
        ...
        if (true === wcf_pro()->utils->is_reference_transaction()) {
            ...
            if (true === wcf_pro()->utils->is_reference_transaction() && $data && isset($_POST['_wcf_checkout_id'])) {
                $data['returnurl'] = add_query_arg(['create-billing-agreement' => true], $data['SECFields']['returnurl']);
                ...
                try {
                    ...
                    if (1 == $express_checkout_details_response['BILLINGAGREEMENTACCEPTEDSTATUS']) {
                        ...
                        if ($order->get_total() > 0) {
                            $billing_agreement_response = $this->perform_express_checkout_request($token, $order, [
                                'payment_action' => 'Sale',
    ...
    public function process_offer_payment($order, $product) {
        ...
        try {
            ...
            $this->add_reference_trans_args($this->get_token($order), $order, [], $product);
     ...
     * @since 1.0.0
     */
    public function add_reference_trans_args($reference_id, $order, $args = [], $offer_product) {
        $defaults = [
        ...
        if (!is_null($refund_amount) && class_exists('WC_Gateway_Paypal')) {
            ...
            if (isset($available_gateways['paypal'])) {
                ...
                if (is_wp_error($result)) {
                    wcf()->logger->log("Paypal offer refund failed. Order: {$order_id}, Error: " . wp_json_encode($result->get_error_message()));
                } else {
                ...
                if (isset($result->L_LONGMESSAGE0)) { //phpcs:ignore
                    wcf()->logger->log('Paypal Express Checkout offer refund error message: ' . wp_json_encode($result->L_LONGMESSAGE0)); //phpcs:ignore
                }
            }
