...
class AngellEYE_PFW_Payment_Logger {
    ...
    public function __construct() {
        ...
        $this->allow_method = [
            ...
            'ProcessTransaction', 
            'Braintree', 
            'PayPal Credit Card (REST)'
        ];
        
        add_action('angelleye_paypal_response_data', [$this, 'own_angelleye_paypal_response_data'], 10, 6);
    }
    ...
    public function own_angelleye_paypal_response_data($result_data, $request_data, $product_id = 1, $sandbox = false, $is_nvp = true, $payment_method = 'express_checkout') {
        ...
        if (!empty($result_data) && is_array($result_data) && !empty($result_data['CURL_ERROR'])) {
            return $result_data;
        } else {
            if ($is_nvp) {
            ...
            if (!empty($result) && is_array($result) && !empty($result['PNREF']) && (isset($request['TRXTYPE[1]']) && $request['TRXTYPE[1]'] != 'I')) {
                $request['METHOD'] = 'ProcessTransaction';
            }
