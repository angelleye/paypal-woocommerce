...
class WC_Gateway_PayPal_Express_Function_AngellEYE {
    ...
    public function express_checkout_get_option($option_name) {
        try {
            ...
            if (!empty($woocommerce_express_checkout_settings[$option_name])) {
    ...
    public function ec_get_checkout_url($action) {
        return add_query_arg(['pp_action' => $action, 'utm_nooverride' => '1'], untrailingslashit(WC()->api_request_url('WC_Gateway_PayPal_Express_AngellEYE')));
    }

    public function ec_is_express_checkout() {
        $paypal_express_checkout = angelleye_get_session('paypal_express_checkout');
        
        if (!empty($paypal_express_checkout['token']) && !empty($paypal_express_checkout['payer_id'])) {
            return true;
        } else {
            return false;
        }
