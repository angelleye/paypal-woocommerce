...
class Angelleye_PayPal_Express_Checkout_Helper {
    ...
    public $enabled;
    public $paypal_flow_setting;
    public $paypal_pro_setting;
    public $paypal_flow_enabled;
    public $paypal_pro_enabled;
    ...
    public $review_title_page;
    public $show_on_checkout;
    public $button_position;
    public $show_on_cart;
    public $show_on_minicart;
    public $prevent_to_add_additional_item_value;
    public $prevent_to_add_additional_item;
    public $testmode_value;
    public $testmode;
    public $billing_address_value;
    public $disallowed_funding_methods;
    public $button_size;
    public $button_height;
    public $button_color;
    public $button_shape;
    public $button_label;
    public $button_tagline;
    public $button_layout;
    public $single_product_configure_settings_value;
    public $single_product_configure_settings;
    public $cart_configure_settings_value;
    public $cart_configure_settings;
    public $mini_cart_configure_settings_value;
    public $mini_cart_configure_settings;
    public $checkout_page_configure_settings_value;
    public $checkout_page_configure_settings;
    public $wsc_cart_configure_settings_value;
    public $wsc_cart_configure_settings;
    public $wsc_cart_disable_smart_button;
    public $billing_address;
    public $cancel_page;
    public $order_review_page_custom_message;
    public $use_wp_locale_code;
    public $enable_in_context_checkout_flow;
    public $is_paypal_credit_enable;
    public $show_paypal_credit;
    public $enable_google_analytics_click;
    ...
    public $disable_term_value;
    public $disable_term;
    public $checkout_page_disable_smart_button_value;
    public $checkout_page_disable_smart_button;
    public $enabled_credit_messaging_value;
    ...
    public function __construct() {
        try {
            ...
            if ($this->enabled == 'yes') {
                ...
                add_filter('angelleye_paypal_checkout_sdk_components', array($this, 'angelleye_paypal_checkout_sdk_components'), 10, 1);
                ...
                add_filter('woocommerce_cart_shipping_packages', array($this, 'maybe_add_shipping_information'), 11, 1);
    ...
    public function ec_enqueue_scripts($is_mini_cart = false) {
        ...
        try {
            ...
            if ($this->is_angelleye_paypal_sdk_required() == false && $this->is_angelleye_product_page() == false && (!isset(WC()->cart) || WC()->cart->needs_payment() == false )) {
                if ($this->show_on_cart == 'no' && $this->show_on_minicart == 'no') {
                    if (apply_filters('is_angelleye_paypal_sdk_required', false) === false) {
                        return false;
                    }
                }
            }
            ...
            if ($this->angelleye_is_in_context_enable() == true || $this->is_angelleye_paypal_sdk_required()) {
                ...
                // Removed direct JS SDK Load to speed up the initial page load performance
                ...
                wp_register_script('angelleye_ppcp-common-functions', PAYPAL_FOR_WOOCOMMERCE_ASSET_URL . 'ppcp-gateway/js/wc-angelleye-common-functions' . $this->minified_version . '.js', array('jquery', 'wp-i18n'), $script_versions, false);
        ...
        if ('woocommerce_widget_cart' === $widget_id) {
            if (AngellEYE_Utility::is_express_checkout_credentials_is_set()) {
                if ($this->enable_in_context_checkout_flow === 'yes' && $this->enabled == 'yes' && $this->show_on_minicart == 'yes') {
                    wp_enqueue_script('angelleye_ppcp-common-functions');
                    wp_print_scripts('angelleye-paypal-checkout-sdk');
    ...
    public function angelleye_in_content_js($tag, $handle) {
        if ('angelleye-paypal-checkout-sdk' === $handle) {
            $tag = str_replace(' src=', ' data-namespace="angelleye_paypal_sdk" src=', $tag);
        }
        return $tag;
    }
    ...
    public function angelleye_paypal_checkout_sdk_components($components) {
        if ($this->enabled_credit_messaging) {
            if (!empty($components)) {
                if (!in_array('messages', explode(',', $components))) {
                    $components_array = explode(',', $components);
                    array_push($components_array, "messages");
                    $components = implode(',', $components_array);
                }
            }
        }
        return $components;
