...
class WC_Gateway_PayPal_Express_Request_AngellEYE {
    ...
    public function angelleye_redirect_action($url) {
        if (!empty($url)) {
            if (!empty($_REQUEST['request_from']) && $_REQUEST['request_from'] == 'JSv4') {
                ...
                wp_send_json(['token' => wc_clean($query_params['token'])]);
    ...
    public function angelleye_do_express_checkout_payment() {
        try {
            ...
            $payment_meta = [
                'Payment type'              => !empty($this->paypal_response['PAYMENTINFO_0_PAYMENTTYPE']) ? $this->paypal_response['PAYMENTINFO_0_PAYMENTTYPE'] : '', 
                'PayPal Transaction Fee'    => !empty($this->paypal_response['PAYMENTINFO_0_FEEAMT']) ? $this->paypal_response['PAYMENTINFO_0_FEEAMT'] : ''
            ];
            ...
            if ($this->response_helper->ec_is_response_success($this->paypal_response)) {
                ...
                apply_filters('woocommerce_payment_successful_result', ['result' => 'success'], $order_id);
    ...
    public function angelleye_do_express_checkout_payment_request() {
        try {
            ...
            if ($this->send_items) {
                ...
                $this->order_param = ['is_calculation_mismatch' => true];
            }
            ...
            $REVIEW_RESULT = !empty($paypal_express_checkout['ExpresscheckoutDetails']) ? $paypal_express_checkout['ExpresscheckoutDetails'] : [];
            
            $PaymentRedeemedOffers = [];
    ...
    public function angelleye_set_express_checkout_request() {
        ...
        try {
            ...
            if (!empty($_REQUEST['request_from']) && $_REQUEST['request_from'] == 'JSv4') {
                $returnurl = urldecode(add_query_arg(['pp_action' => 'get_express_checkout_details', 'utm_nooverride' => 1, 'request_from' => 'JSv4'], untrailingslashit(WC()->api_request_url('WC_Gateway_PayPal_Express_AngellEYE'))));
            } else {
                $returnurl = urldecode(add_query_arg(['pp_action' => 'get_express_checkout_details', 'utm_nooverride' => 1], untrailingslashit(WC()->api_request_url('WC_Gateway_PayPal_Express_AngellEYE'))));
            }
            ...
            if (!empty($_GET['pay_for_order']) && $_GET['pay_for_order'] == true && !empty($_GET['key'])) {
                ...
                if ($this->send_items) {
                    ...
                    $this->cart_param = ['is_calculation_mismatch' => true];
                }
                ...
                if (!empty($_REQUEST['request_from']) && $_REQUEST['request_from'] == 'JSv4') {
                    $returnurl = urldecode(add_query_arg([
        ...
        try {
            ...
            switch (strtolower($payment_status)) :
                case 'completed' :
                    ...
                    if (!in_array(strtolower($transaction_type), ['merchtpmt', 'cart', 'instant', 'express_checkout', 'web_accept', 'masspay', 'send_money', 'expresscheckout', 'paypal_here'])) {
                        break;
                    }
    ...
    public function angelleye_add_extra_order_meta($order) {
        ...
        if (!empty($this->gateway->payment_action) && $this->gateway->payment_action != 'Sale') {
            ...
            if (!class_exists('WooCommerce') || WC()->session == null) {
                $payment_order_meta = [
                    '_payment_action' => $this->gateway->payment_action, 
                    '_first_transaction_id' => isset($this->paypal_response['PAYMENTINFO_0_TRANSACTIONID']) ? $this->paypal_response['PAYMENTINFO_0_TRANSACTIONID'] : $this->paypal_response['TRANSACTIONID']
                ];
            } else {
                $paypal_express_checkout = angelleye_get_session('paypal_express_checkout');
                ...
                $payment_order_meta = [
                    '_payment_action' => $this->gateway->payment_action, 
                    '_express_checkout_token' => $paypal_express_checkout['token'], 
                    '_first_transaction_id' => isset($this->paypal_response['PAYMENTINFO_0_TRANSACTIONID']) ? $this->paypal_response['PAYMENTINFO_0_TRANSACTIONID'] : $this->paypal_response['TRANSACTIONID']
                ];
            }
    ...
    public function DoReferenceTransaction($order_id) {
        ...
        $PaymentDetails = [
            ...
            'custom' => apply_filters('ae_ppec_custom_parameter', json_encode(['order_id' => version_compare(WC_VERSION, '3.0', '<') ? $order->id : $order->get_id(), 'order_key' => version_compare(WC_VERSION, '3.0', '<') ? $order->order_key : $order->get_order_key()])), // Free-form field for your own use.  256 char max.
        ...
        if ($order->needs_shipping_address()) {
            ...
            $ShippingAddress = ['shiptoname' => wc_clean(stripslashes($shipping_first_name . ' ' . $shipping_last_name)), // Required if shipping is included.  Person's name associated with this address.  32 char max.
    ...
    public function get_token_by_token($token_id, $token_result = null) {
        ...
        if (is_null($token_result)) {
            $token_result = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_payment_tokens WHERE token = %s", $token_id));
            ...
            if (!empty($gateways)) {
                ...
                if (isset($PayPalResult['ACK']) && $PayPalResult['ACK'] == 'Success') {
                    if (isset($PayPalResult['PAL']) && !empty($PayPalResult['PAL'])) {
                        ...
                        update_option('angelleye_express_checkout_default_pal', ['Sandbox' => $this->testmode, 'APIUsername' => $this->api_username, 'PAL' => $merchant_account_id]);
