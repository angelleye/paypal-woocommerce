...
class AngellEYE_Admin_Order_Payment_Process {
    ...
    public function __construct() {
        if (is_admin() && !defined('DOING_AJAX')) {
            add_action('add_meta_boxes', [$this, 'angelleye_add_meta_box'], 10);
            add_action('woocommerce_process_shop_order_meta', [$this, 'angelleye_admin_create_reference_order'], 10, 2);
            add_action('woocommerce_process_shop_order_meta', [$this, 'angelleye_admin_order_process_payment'], 10, 2);
            add_action('angelleye_admin_create_reference_order_action_hook', [$this, 'angelleye_admin_create_reference_order_action'], 10, 1);
            add_action('angelleye_admin_order_process_payment_action_hook', [$this, 'angelleye_admin_order_process_payment_action'], 10, 1);
        }
    }

    public function angelleye_add_meta_box() {
        add_meta_box('angelleye_admin_order_payment_process', __('Reference Transaction', 'paypal-for-woocommerce'), [$this, 'admin_order_payment_process'], 'shop_order', 'side', 'default');
        add_meta_box('angelleye_admin_order_reference_order', __('Reference Transaction', 'paypal-for-woocommerce'), [$this, 'admin_order_reference_order'], 'shop_order', 'side', 'default');
    }
    ...
    public function angelleye_admin_create_reference_order_action($order) {
        ...
        if (in_array($this->payment_method, ['paypal_express', 'paypal_pro', 'paypal_pro_payflow'])) {
    ...
    public function angelleye_get_payment_token($user_id, $order) {
        ...
        if (in_array($this->payment_method, ['paypal_pro', 'paypal_pro_payflow'])) {
    ...
    public function angelleye_load_payment_method_setting($order) {
        ...
        switch ($this->payment_method) {
            case (in_array($this->payment_method, ['paypal_express', 'paypal_pro', 'paypal_pro_payflow'])):
        ...
        if (!empty($token_id)) {
            ...
            if (!empty($result['ACK']) && ($result['ACK'] == 'Success' || $result['ACK'] == 'SuccessWithWarning')) {
                ...
                if ($this->gateway_settings['payment_action'] != 'Sale') {
                    ...
                    $payment_order_meta = [
			    '_payment_action' 		=> $this->gateway_settings['payment_action'], 
			    '_first_transaction_id' 	=> $result['TRANSACTIONID']
		    ];
	...
	$PaymentDetails = [];
        ...
        $PaymentDetails = [
            ...
            'custom' 		=> apply_filters('ae_ppec_custom_parameter', json_encode(['order_id' => $order_id, 'order_key' => version_compare(WC_VERSION, '3.0', '<') ? $order->order_key : $order->get_order_key()])),
